...
export const metadata: Metadata = { title: 'PBAT Achievements' };

interface Props { searchParams: Promise<{ page?: string }> }
const PER_PAGE = 12;

export default async function PbatPage({ searchParams }: Props) {
  const { page: pageParam } = await searchParams;
  const page = Math.max(1, parseInt(pageParam || '1', 10));
  await connectDB();

  const newsCat = await Category.findOne({ title: 'News' }).lean();
  const filter = newsCat ? { category: { $ne: newsCat._id }, status: '1' } : { status: '1' };
  ...
  const [total, posts, categories, latest] = await Promise.all([
    ...
    Category.find({ title: { $ne: 'News' } }).sort({ title: 1 }).lean(),
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 grid lg:grid-cols-[1fr_320px] gap-10">
      <div>
        <h1 className="text-3xl font-bold mb-6">PBAT Achievements</h1>
        {posts.length === 0 ? (
          <p className="text-gray-600">No achievements posted yet.</p>
        ) : (
        ...
        {totalPages > 1 && (
          <div className="flex gap-2 justify-center mt-8">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
              <a key={n} href={`/pbat?page=${n}`} className={`px-3 py-1 rounded border ${n === page ? 'bg-[var(--tsg-green)] text-white' : 'bg-white'}`}>{n}</a>
            ))}
          </div>
        )}
      </div>
